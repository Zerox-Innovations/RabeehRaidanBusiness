        ...
        <div className='flex flex-col'>
          ...
          <div className="space-y-8 grid md:grid-cols-1 lg:grid-cols-2 gap-16">
            {branches.map((branch, index) => (
              <div
                key={index}
                className="flex flex-col md:flex-row items-center md:space-x-6 space-y-6 md:space-y-2"
              >
                {/* Image */}
                <img
                  src={branch.image}
                  alt={branch.name}
                  className="max-w-xl sm:max-w-lg md:max-w-5xl sm:h-48 md:h-64 object-cover rounded-lg"
                />
                {/* Branch Details */}
                <div className="flex-1">
                  ...
                  <button onClick={branch.mapLink} className='bg-blue-500 p-3 text-white rounded-xl font-bold'>Navigate me</button>
                </div>
              </div>
            ))}
          </div>
